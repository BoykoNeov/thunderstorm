...
const key = (over: Partial<ViewKey> = {}): ViewKey => ({
  az: 1, el: 2, dist: 3, fovY: 4, targetX: 0, targetY: 0, targetZ: 5,
  fa: 6, fb: 7, mix: 0.5, xsec: 0, xpos: 0.5, layer: 0, ...over,
});
...
describe("sameView", () => {
  ...
  it("is false when any single field differs", () => {
    // targetX/targetY are load-bearing: a right-drag pan (slice 5c) that moved
    // only in x/y would otherwise average two framings into one still.
    const fields: (keyof ViewKey)[] = [
      "az", "el", "dist", "fovY", "targetX", "targetY", "targetZ",
      "fa", "fb", "mix", "xsec", "xpos", "layer",
    ];
