...
## Phase 3 T6 and T7 — close-out, 2026-09-06/07

Full document: `docs/phase3-completion-2026-09-06.md`. This entry is the log record.

### T6 — VOID AS SPECIFIED, by measurement

The plan's T6 row is *"Multicell run + export + diorama"*, and the 2026-09-02 amendment
pinned its asset exactly: *"the asset is T5s's **confirmed** multicell at 333 m."*
**That asset does not exist, and T5s is the reason.** The external-sounding path works
and the environment reaches the multicell regime, but no multicell *label* was
reachable: the persistence criterion sat at its 80-minute ceiling for every sheared
storm at 1 km **and** at 500 m, and the one piece of multicell-side evidence any member
had did not survive refinement.

So T6's input was measured unreachable, not merely un-run. **No substitute asset was
shipped**, deliberately: the only candidate cannot honestly be *named* multicell, and
shipping it under that name would put a label in a teaching package that the project's
own classifier refuses. The other three items in the T6 row are void for reasons of
their own — the export box for a broad multi-updraft field was solved anyway by the
squall-line contract (§§4.4–4.4a, `test_squall_box.py` 27/27), and the diorama picker
has been scenario-agnostic since Phase 2 T7, so a new package needs no viewer work.

### T7 — the fork-neutrality gate: RAN, branch 1, all three comparisons IDENTICAL

**Pre-registered first.** §2 of the completion doc — claim, available baselines, the
hazard, the design, the three comparisons and **four** branches — was committed
(`487221c`) before the gate ran, and amended once mid-flight (`f63fbf5`, §2.6a) before
any of its output was hashed. The branches were not renegotiated.

**The claim under test.** T4 §4.1 recorded, honestly, that *"the fork is bitwise-neutral
for every `irandp=0` run at any grid and rank count"* was **an extrapolation from one
datum** (500 m, 60×60, np=4) and carried the check to T7. The patched block is
unreachable at `irandp=0`, so neutrality is *expected* — but the patch adds lines to a
source file, and in principle that can move compiler decisions in the enclosing routine.
This is load-bearing: `runs/cm1.exe` is a symlink into the build tree, so the charter's
recovery path (*"regeneration from `sim/` + `pipeline/`"*) now runs on the **fork**.

**What made it measurable at all:** the raw netCDF for all three shipped scenarios
turned out to have survived on `~/thunderstorm/runs/` — 240 GB the charter calls
*disposable by design*. That is a better baseline than T4 envisaged (it wanted recorded
checksums, which did not exist) and a **fragile** one, which is why §2.6a exists.

**Design: two fresh runs, one deck.** `single_cell_500m` at 160×160×40, np=8 — differing
from T4's gate in **both** quantities its scope named (grid and rank count). One deck
generated by `pipeline/gen_deck.py`, `--verify`'d against the July run's own
`namelist.input` (344/344 keys), handed to both members; both recorded
`namelist_sha256 = 4eef9f40b4a0e679…`, so **the only difference between the two members
was the binary**. Three refusals cleared before either started: both binary hashes
asserted *and asserted to differ* (so the gate could not pass trivially), the baseline
present, and neither target directory colliding with any scenario's declared `run_dir` —
`sim/run_scenario.sh` wipes its run dir on start, and the July baseline is irreplaceable.

**Result** (`sim/gates/t7_neutrality.sh`, 2026-09-06 23:02 → 2026-09-07 01:01):

| # | Comparison | Result |
|---|---|---|
| 1 | A(fork) vs B(stock), **both today** | **IDENTICAL** (302 files) |
| 2 | B(stock) vs July on-disk | **IDENTICAL** (302 files) |
| 3 | A(fork) vs July on-disk | **IDENTICAL** (302 files) |

Comparison 3 was measured rather than inferred, which makes it the transitivity check:
1 and 2 both IDENTICAL *force* 3, so a differing 3 would have meant a broken comparator.

**T4 §4.1 is upgraded from *expected* to MEASURED TWICE**, and `docs/phase3-t4-seed.md`
now says so at the paragraph that made the promise. Two free results came with it:

- The generated deck is **byte-different** from July's hand-written one (`4eef9f40…` vs
  `8244367c…`) and only *value*-identical — and the run came back bitwise identical
  anyway. So comments and key ordering are now **measured** not to reach the model, which
  retires a standing worry about whether `--verify`'s parsed-value comparison is strong
  enough. No amount of reading the comparator could have settled that.
- Wall clock ran 68 m 22 s (fork) against 48 m 05 s (stock) — a ~42 % spread from a
  competing background job, with identical bytes. Stated narrowly: neither the patch nor
  a large contention swing moved a byte. **Not** a claim that the machine is
  deterministic in general — that needs an A/A pair, which §2.6 left as the
  1-FAIL-and-2-FAIL escalation and which did not fire.

**Scope, carried forward so a clean PASS does not read wider than it is:** one scenario,
one grid, one rank count. `supercell_333m` (540², 218 GB, ~4.5 h) stays unmeasured;
closing it is now a known run, not a new design, and it is an owner call.

### `sim/baselines/` — the artifact §2.6a promised, and why it is not tidiness

The 1-PASS/2-FAIL branch promised a downgrade to *"statistical equivalence + recorded
output checksums"* — and §2.2 had already established that **no such checksums existed**.
The branch pointed at a fallback with no artifact behind it. Caught mid-flight and fixed
by amendment before any result was read: the gate now creates the artifact.

`sim/baselines/` holds one sorted `sha256` line per output file for all three shipped
scenarios (1206 files, ~100 KB). The point is that **the only baseline for any
reproducibility claim in this project is 240 GB of raw netCDF the charter explicitly
calls disposable** — the first time anyone reclaims that disk, this gate becomes
permanently un-re-runnable, not only for this fork but for every future compiler, MPI or
netCDF bump. Provenance (grid, rank count, producing binary, file count) was **read off
each run's own `namelist.input` and `run_meta.txt`**, not carried from a plan; the file
also spells out that "grid" means the CM1 simulation grid and not the export crop box,
because `supercell_333m` simulates on 40 stretched levels and exports 54 voxels, and a
provenance file whose provenance was assumed is exactly what this directory exists to
prevent.

Both gate members' `run_meta.txt` are preserved at
`sim/gates/t7_neutrality.{fork,stock}.run_meta.txt`. The 12.2 GB of output they describe
was deleted; those two files plus the checksum lists are what makes §3 a record rather
than a claim.

### Also delivered under T7

- **A deck rule written before the phase that needs it.** `terrain_flag`/`itern` with
  `imove ≠ 0` is now refused by `pipeline/cm1post/deck.py` (Category 3). The failure mode
  is why it is worth a guard: a moving domain plus terrain does **not** crash CM1 — it
  produces a run in which the ground slides beneath the storm, with nothing in the output
  saying so. `test_deck.py` goes 16 → **20**: two refusals (both routes into terrain) and
  two controls (terrain with `imove=0` still generates; motion without terrain still
  generates), so it is a targeted rule and not a blanket ban on either.
- **`base.F` line numbers recorded where the pin lives.** `sim/cm1-patches/README.md`
  gains a section of source facts read against the pinned tarball and explicitly **not
  patched** — the `isnd=7` file format, `iwnd` being ignored (and `param.F` warning
  non-fatally where this project refuses), the real level constraints, `u0`/`v0` being
  grid-relative, and the dead `input_sounding_grid`. These are the parts of stock CM1 the
  project depends on, so they are what must be re-checked after any version bump.
- **One hole found and not chased:** `t4gate` and `t4gate_sc` carry no `run_meta.txt`,
  because they were made outside `sim/run_scenario.sh`. So that README's *"every run says
  which binary produced it"* holds for production runs and not for hand-made gate
  directories. This gate wrote its own for both members precisely because it is not
  automatic.

### Phase 3 state

**CLOSED, not COMPLETE** — the Phase 1 precedent. T1, T3, T4, T5s and T7 delivered;
**T5 closed as measured** (no multicell reachable from the namelist) and **T6 void by
measurement**. Five exit options are written up in the completion doc §5, none started,
each an owner call.
